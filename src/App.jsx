      ...
      <header className="store-header">
        <div className="container store-nav">
          ...
          <nav className="store-links">
            ...
            <button onClick={() => go(authUser ? "/account" : "/login")}>{authUser ? "حسابي" : "دخول العميل"}</button>
            <button onClick={() => go("/admin")}>لوحة التحكم</button>
          </nav>
          <button className="cart-main" onClick={() => setCartOpen(true)}><ShoppingBag size={18}/> السلة {cartCount ? <strong>{cartCount}</strong> : null}</button>
        </div>
      </header>
...
function Account({ customer, setCustomer, go }) {
  ...
  return (
    <div className="store account-page" dir="rtl">
      <header className="store-header">
        <div className="container store-nav">
          <button className="logo" onClick={() => go("/")}><b>حسابي</b><span>Customer Profile</span></button>
          <nav className="store-links"><button onClick={() => go("/")}>المتجر</button><button onClick={() => signOut(auth)}>تسجيل خروج</button></nav>
        </div>
      </header>
